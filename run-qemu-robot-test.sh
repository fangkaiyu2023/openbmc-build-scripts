...
#DOCKER_IMG_ID="$(docker images -q "${DOCKER_IMG_NAME}":latest 2> /dev/null)"
...
if [[ ${LAUNCH} == "local" ]]; then
    ...
    #Now wait for the OpenBMC QEMU Docker instance to get to standb
    delay=5
    attempt=$(( QEMU_LOGIN_TIMER / delay ))
    while [ $attempt -gt 0 ]; do
        attempt=$(( attempt - 1 ))
        # docker start "$obmc_qemu_docker"
    ...
    # Now stop and delete the QEMU Docker contariner
